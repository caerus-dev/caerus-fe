...
function parseAndTranslateError(raw: any, templateName?: string, failedEnvNames: string[] = []): string {
  ...
  if (typeof msg === "string") {
    if (msg.includes("already exists")) {
      return `Ya existe una plantilla de recurso con el nombre "${templateName || ''}" en ${envsLabel}.`
    }
  ...
  const handleDuplicate = async () => {
    ...
    try {
      ...
      await Promise.all(
        selectedEnvIds.map(async (targetEnvId) => {
          ...
          const payload = {
            ...
            defaultTtlSec: Number(template.defaultTtlSec || 3600),
            conflictResolution: (template.conflictResolution || "WAIT").toUpperCase(),
            retryIntervalSec: template.retryIntervalSec ? Number(template.retryIntervalSec) : null,
            maxRetryCount: template.maxRetryCount ? Number(template.maxRetryCount) : null,
            useIdempotency: Boolean(template.useIdempotency),
            saveMetadata: Boolean(template.saveMetadata),
            notificationWebhookUrl: template.notificationWebhookUrl || null,
            environmentId: numericEnvId,
          }

          const res = await fetch("/api/shared-resource-templates", {
            method: "POST",
          ...
          if (res.ok) {
            ...
            console.error(`Duplicate template error for env ${envName}:`, rawMsg)
      ...
      if (failedEnvsMap.length > 0) {
        ...
        setErrorMessage(parseAndTranslateError(firstError, template?.name, failedNames))
        if (successfulEnvs.length > 0) {
          const successNames = successfulEnvs.map((s) => s.name).join(", ")
          setSuccessMessage(`Se duplicó exitosamente en: ${successNames}`)
          toast.success(`Recurso "${template?.name}" duplicado exitosamente en: ${successNames}`)
        }
      } else {
        const successNames = successfulEnvs.map((s) => s.name).join(", ")
